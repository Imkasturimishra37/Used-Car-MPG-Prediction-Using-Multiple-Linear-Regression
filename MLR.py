# -*- coding: utf-8 -*-
"""
Created on Tue May 19 10:10:12 2026
...
The customers in this sector give strong preference to less-aged cars and popular brands with good resale value. 
This puts a very strong challenge as they only have a very limited range of vehicle options to showcase.

#### No Pre-Set Standards

`How does one determine the value of a used car?`

The Market scenario is filled with a lot of malpractices. There is no defined standards exist to determine the appropriate price
 for the cars, the values are determined by arbitrary methods.

The unorganized and unstructured methods are disadvantageous to the both the parties trying to strike a deal. 
The look and feel can be altered in used cars, but the performance cannot be altered beyond a point.


#### Revolutionizing the Used Car Industry Through Machine Learning

**Linear regression**
Linear regression is a ML model that estimates the relationship between independent variables and a dependent variable using a 
linear equation (straight line equation) in a multidimensional space.
...
cars = pd.read_csv(r"C:\Users\ADMIN\Desktop\Data science practice\Machine_learning\All New DataSets\All New DataSets\Data sets\CarswithEnginetype.csv")
...
X.plot(kind = 'box', subplots = True, sharey = False, figsize = (20, 18)) 
...
## Build the best Model building without cv
...
# linerRegresison function initializes a linear regression model
lm = LinearRegression()

## Scores with KFold, defines a 5-fold cross-validation strategy with shuffling of the data and a random seed of 100 for reproducibility.
folds = KFold(n_splits = 5, shuffle = True, random_state = 100)
# cross_val_score performs cross-validation on the training data (X_train, Y_train) using the linear regression model (lm) with 5 folds and computes the R-squared scores for each fold.
...
'''
## Testing
data = pd.read_csv(r"D:\StudyMaterial\ML\MultipleLinear regression\Codes\Multiple_Linear Regression\Multiple_Linear Regression\Cars_test.csv")
